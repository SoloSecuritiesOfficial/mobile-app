  ...
  const handleSubmitFlag = async () => {
    ...
    try {
      ...
      const res = await api.post(
        `/ctf/${selectedChallenge.challengeId}/submit`,
        { flag: flagInput.trim() },
      );
      if (res.data?.success) {
        Alert.alert(
          "🎉 Correct!",
          `${res.data.isFirstBlood ? "🩸 First Blood! " : ""}You earned ${res.data.xpEarned} XP!`,
        );
      ...
      if (
        ...
        err?.response?.data?.premiumRequired
      ) {
        Alert.alert(
          "👑 Premium Required",
          err?.response?.data?.message ||
            "This challenge requires a Premium subscription.",
          [{ text: "OK" }],
        );
        setSelectedChallenge(null);
      } else {
        Alert.alert(
          "❌ Incorrect",
          err?.response?.data?.message || "Wrong flag. Try again!",
        );
      }
    } finally {
      setSubmitting(false);
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.statPill}>
            <Text style={styles.statPillNum}>
              {solved.reduce((acc, c) => acc + c.points, 0)}
            </Text>
      ...
      >
              ...
              {categories.map((cat) => (
                ...
                >
                  <Text
                    style={[
                      styles.catBtnText,
                      filterCategory === cat && styles.catBtnTextActive,
                    ]}
                  >
                    {cat}
                  </Text>
            ...
            {filtered.length === 0 ? (
              <View style={styles.emptyBox}>
                ...
                <Text style={styles.emptyText}>
                  Challenges will be added by the admin. Check back soon!
                </Text>
              </View>
            ) : (
              filtered.map((c) => {
                ...
                return (
                  ...
                  >
                      ...
                      <View style={{ alignItems: "flex-end", gap: 4 }}>
                        <View
                          style={[
                            ...
                            {
                              backgroundColor:
                                DIFFICULTY_COLORS[c.difficulty] + "22",
                            },
                          ]}
                        >
                          <Text
                    ...
                    <View style={styles.cardBottom}>
                      <Text style={styles.cardSolves}>
                        👥 {c.solves?.length ?? 0} solves
                      </Text>
                      {isSolved && (
        ...
        {tab === "leaderboard" &&
          (leaderboard.length === 0 ? (
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyText}>
                Be the first to solve a CTF challenge!
              </Text>
            </View>
          ) : (
            leaderboard.map((u, i) => (
        ...
        {tab === "solved" &&
          (solved.length === 0 ? (
            <View style={styles.emptyBox}>
              <Text style={styles.emptyIcon}>🔓</Text>
              <Text style={styles.emptyTitle}>No Solved Challenges</Text>
              <Text style={styles.emptyText}>
                Solve your first CTF challenge to see it here.
              </Text>
            </View>
          ) : (
            solved.map((c) => (
